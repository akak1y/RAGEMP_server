...
const handleEscapeClose = (event) => {
    ...
    for (const winName in windows.value) {
        // ищем открытое окно
        if (windows.value[winName] === true) {
            windows.value[winName] = false;
            setTimeout(() => {
                if (focusTrap.value) {
                    // снимаем фокус через 150 мс
    ...
    window.updateInventory = (slotsJson, configJson) => {
        try {
            ...
            if (parsedSlots && itemConfig) {
                ...
                inventory.value = parsedSlots || new Array(20).fill(null);
            } // если пришёл сломанные данные
        } catch (e) {
    ...
    window.setDealershipCars = (serverConfigJson) => {
        try {
            dealershipCars.value =
                typeof serverConfigJson === 'string'
                    ? JSON.parse(serverConfigJson)
                    : serverConfigJson;
        } catch (e) {
