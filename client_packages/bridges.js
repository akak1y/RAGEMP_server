...
mp.events.add("client:updateMoney", (money) => {
    if (state.uiBrowser) state.uiBrowser.execute(`if(window.updateMoney) window.updateMoney(${money});`);
});
...
mp.events.add("client:setRedisStats", (count) => {
    if (state.uiBrowser) state.uiBrowser.execute(`if(window.updateGlobalStats) window.updateGlobalStats(${count});`);
});
...
mp.events.add("client:phone:requestPriceDeliveryCar", (price) => {
    if (state.uiBrowser) state.uiBrowser.execute(`if(window.setPriceDeliveryCar) window.setPriceDeliveryCar(${price});`);
});
