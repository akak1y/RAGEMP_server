...
describe('auth', () => {
  ...
  beforeEach(() => {
    ...
    state.openWindowsState = { inventory: true, phone: true, dealership: true, carCustom: true };
  ...
  test('authError: показывает ошибку в браузере', () => {
    ...
    expect(state.uiBrowser.execute).toHaveBeenCalledWith('window.showAuthError("Неверный пароль!")');
  });
  ...
  test('authError: кавычки в сообщении экранируются безопасно', () => {
    ...
    expect(state.uiBrowser.execute)
      .toHaveBeenCalledWith(`window.showAuthError(${JSON.stringify(msg)})`);
  });
  ...
  test('hideAuth: сбрасывает блокировки и запрашивает позиции локаций', () => {
    ...
    expect(state.openWindowsState).toEqual({
      inventory: false, phone: false, dealership: false, carCustom: false
    });
