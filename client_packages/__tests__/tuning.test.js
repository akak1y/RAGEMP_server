...
describe('tuning: server-authoritative', () => {
  ...
  afterEach(() => { mp.players.local.vehicle = null; });

  test('applyUpgrade: запрос уходит на сервер', () => {
    mp.events.__trigger('client:custom:applyUpgrade', 'wheels', '{"wheelType":0,"wheelId":5}', 1000);
    expect(mp.events.callRemote)
      .toHaveBeenCalledWith('server:custom:buyUpgrade', 'wheels', '{"wheelType":0,"wheelId":5}', 1000);
  });
