...
function start() {
        ...
        if (req.method === 'POST' && req.url === '/login') {
            ...
            req.on('end', async () => {
                try {
                    ...
                    const token = jwt.sign({ accountId: r.user.id, username: r.user.username, adminLevel: r.user.admin_level }, JWT_SECRET, { expiresIn: '8h' });
    ...
    wss.on('connection', (socket, req) => {
        ...
        let payload;
        try { payload = jwt.verify(url.searchParams.get('token'), JWT_SECRET); }
    ...
    setInterval(() => {
        const players = mp.players.toArray()
            ...
            .map(p => ({
                id: p.accountId, name: p.accountName,
                x: p.position.x, y: p.position.y, z: p.position.z, heading: p.heading
            }));
