...
mp.events.add('server:phone:spawnVehicle', withGuards([isLoggedIn, rateLimit('spawn_vehicle', 1, 3)], async (player, vehicleDbId, fromPhone) => { // доставка авто
    ...
    if (fromPhone) {
        ...
        if (!paid) return player.outputChatBox("!{#FF3333}[Ошибка] У вас недостаточно денег!");
    }
...
mp.events.add('server:fuel:refuel', withGuards([isLoggedIn, rateLimit('refuel', 1, 5)], async (player, vehicleDbId) => {
    ...
    try {
        await sequelize.transaction(async (t) => {
            ...
            const result = await vehicleService.refuelVehicle(vehicleDbId, player.accountId);
        ...
        if (err.message === 'refuel_failed') return player.outputChatBox('!{#FF3333}[Заправка] Не удалось заправить машину.');
        throw err;
    }
