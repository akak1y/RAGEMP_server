...
// АВТОСАЛОН ============
mp.events.add(
    'server:dealership:buy',
    withGuards(
        [isLoggedIn, rateLimit('buy_car', 1, 5)],
        async (player, model) => {
...
mp.events.add(
    'server:dealership:requestConfig',
    withGuards(
        [isLoggedIn],
        (player) => {
...
mp.events.add(
    'server:phone:requestPriceDeliveryCar',
    withGuards(
        [isLoggedIn],
        (player) => {
    ...
    withGuards(
        ...
        async (player, vehicleDbId, fromPhone) => {
            ...
            if (!fromPhone) {
                ...
                posCar.coords = new mp.Vector3(
                    player.position.x + 2,
                    player.position.y,
                    player.position.z
                ); // спавним в x+2 от игрока
...
mp.events.add(
    'server:fuel:refuel',
    withGuards(
        [isLoggedIn, rateLimit('refuel', 1, 5)],
