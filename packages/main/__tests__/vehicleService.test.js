...
describe('VehicleService', () => {
    ...
    describe('getVehicleForOwner', () => {
        test('findOne с двумя условиями', async () => {
            ...
            expect(mockVehicleModel.findOne).toHaveBeenCalledWith({ where: { id: 3, owner_id: 7 } });
    ...
    describe('spawnVehicle', () => {
        test('создание + setVariable + trackOwner', () => {
            const carData = {
                id: 10, owner_id: 7, model: 'adder', fuel: 80,
                color_r: 255, color_g: 0, color_b: 0,
                engine_mod: 3, brakes_mod: -1, transmission_mod: -1, turbo_mod: -1,
                wheel_type: 0, wheel_mod: 5
            };
            const coords = new mp.Vector3(1, 2, 3);
            const veh = vehicleService.spawnVehicle(carData, coords, 90, 0);
            
            expect(global.mp.vehicles.new).toHaveBeenCalledWith('hash_adder', coords, expect.any(Object));
    ...
    describe('respawnVehicle', () => {
        ...
        test('успех: despawn + spawn с теми же координатами', async () => {
            ...
            mockVehicleModel.findByPk.mockResolvedValue({
                ...
                engine_mod: -1, brakes_mod: -1, transmission_mod: -1, turbo_mod: -1,
                wheel_type: 0, wheel_mod: -1
            });
