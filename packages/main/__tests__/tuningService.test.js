...
describe('TuningService', () => {
    ...
    describe('getPrice', () => {
        ...
        test('диски — цена комплекта', () =>
            expect(tuningService.getPrice('wheels', { wheelType: 0, wheelId: 5 })).toBe(1000));
        test('неизвестное — 0', () => {
    ...
    describe('isInstalled', () => {
        ...
        test('диски по паре', () => {
            expect(
                tuningService.isInstalled(baseCar(), 'wheels', { wheelType: 0, wheelId: -1 })
            ).toBe(true);
        });
    });
    ...
    describe('buyUpgrade', () => {
        ...
        test('левый цвет и категория — invalid', async () => {
            expect(
                (await tuningService.buyUpgrade(player, veh, 'color', { r: 9, g: 9, b: 9 })).error
            ).toBe('invalid_option');
            expect((await tuningService.buyUpgrade(player, veh, 'nope')).error).toBe(
                'invalid_category'
            );
        });
        ...
        test('недостаточно денег', async () => {
            ...
            expect((await tuningService.buyUpgrade(player, veh, 'engine')).error).toBe(
                'not_enough_money'
            );
        });
